            ...
            if (!String.IsNullOrWhiteSpace(Path))
            {
                ...
                await CoreApplication.MainView.CoreWindow.Dispatcher.RunAsync(CoreDispatcherPriority.Normal, async () =>
                {
                    try
                    {
                        ...
                        {
                            //Debug.WriteLine("LocalImage exists: " + LocalFileName);
                        ...
                        {
                            //Debug.WriteLine("Image exists: " + Path);
                            ImageToBitmapImage.UriSource = new Uri(Path, UriKind.RelativeOrAbsolute);
                            TaskResult.SetResult(ImageToBitmapImage);
                        }
                        else if (Path.StartsWith("data:"))
                        {
                            if (Path.Length > 200)
                            {
                                Debug.WriteLine("Image data: " + Path);
                        ...
                        {
                            Debug.WriteLine("Image not found: " + Path);
                            TaskResult.SetResult(null);
                        }
                    }
                    catch { TaskResult.SetResult(null); }
