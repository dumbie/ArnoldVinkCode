...
{
	inline std::wstring PathMerge(std::wstring pathLeft, std::wstring pathRight)
	{
		try
		{
	...
	inline std::wstring PathGetFolderKnown(KNOWNFOLDERID knownFolderId)
	{
		try
		{
			PWSTR ppszPath;
	...
	inline std::wstring PathGetFileName(std::wstring fullPath)
	{
		try
		{
	...
	inline std::wstring PathGetFileNameWithoutExtension(std::wstring fullPath)
	{
		try
		{
			std::filesystem::path path(fullPath);
	...
	inline std::wstring PathGetExecutableFile()
	{
		try
		{
	...
	inline std::wstring PathGetExecutableDirectory()
	{
		try
		{
			WCHAR exePath[MAX_PATH]{};
	...
	inline std::vector<std::wstring> PathListFiles(std::wstring pathList, bool recursive)
	{
