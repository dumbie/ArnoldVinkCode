...
{
    public partial class AVImageThumb
    {
        //Functions
        public static BitmapSource GetThumbnail(string filePath, uint thumbnailSize)
        {
            IntPtr sharedBitmapPointer = IntPtr.Zero;
            try
            {
                //Create shellitem instance
                SHCreateItemFromParsingName(filePath, 0, typeof(IShellItem).GUID, out IShellItem shellItemInstance);
                ...
                //Get bitmap instance
                thumbnailCacheInstance.GetThumbnail(shellItemInstance, thumbnailSize, WTS_FLAGS.WTS_EXTRACTINPROC, out ISharedBitmap sharedBitmapInstance, out _, out _);
                if (thumbnailCacheInstance == null)
                {
                ...
                //Get bitmap pointer
                sharedBitmapInstance.GetSharedBitmap(out sharedBitmapPointer);
                if (sharedBitmapPointer == IntPtr.Zero)
                {
                ...
                return Imaging.CreateBitmapSourceFromHBitmap(sharedBitmapPointer, IntPtr.Zero, Int32Rect.Empty, BitmapSizeOptions.FromEmptyOptions());
            }
            catch (Exception ex)
            {
                Debug.WriteLine("Thumbnail failed to load: " + ex.Message);
                return null;
            }
            finally
            {
                SafeCloseImage(ref sharedBitmapPointer);
            }
        }
        ...
        {
            uint GetThumbnail([In] IShellItem pShellItem, [In] uint cxyRequestedThumbSize, [In] WTS_FLAGS flags, [Out, MarshalAs(UnmanagedType.Interface)] out ISharedBitmap ppvThumb, [Out] out WTS_CACHEFLAGS pOutFlags, [Out] out WTS_THUMBNAILID pThumbnailID);
